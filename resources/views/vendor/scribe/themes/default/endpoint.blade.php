...
@if(is_string($response->content) && Str::startsWith($response->content, "<<binary>>"))
                ...
                @else
                    ...
                    <code class="language-json"
                          style="max-height: 300px;">{!! htmlentities($parsed != null ? json_encode($parsed, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : $response->content) !!}</code>
                @endif </pre>
        @endforeach
    @endif
</span>
...
<form id="form-{{ $endpoint->endpointId() }}" data-method="{{ $endpoint->httpMethods[0] }}"
    ...
    @if(count($endpoint->headers))
        ...
        @foreach($endpoint->headers as $name => $example)
                ...
                if ($endpoint->isAuthed() &&
                    $metadata['auth']['location'] == 'header' &&
                    $metadata['auth']['name'] == $name) {
    ...
    @if(count($endpoint->queryParameters))
        ...
        @foreach($endpoint->queryParameters as $attribute => $parameter)
                ...
                $htmlOptions = [];
                if ($endpoint->isAuthed() &&
                    $metadata['auth']['location'] == 'query' &&
                    $metadata['auth']['name'] == $attribute) {
                    $htmlOptions = ['class' => 'auth-value',];
                }
                ?>
            <div style="padding-left: 28px; clear: unset;">
