        ...
        function tryItOut(btnElement) {
            ...
            return preflightPromise.then(() => makeAPICall(method, path, body, query, headers, endpointId))
                                   .then(([responseStatus, statusText, responseContent, responseHeaders]) => {
                                       ...
                                       responsePanel.querySelector(`.response-status`).textContent = responseStatus +
                                                                                                     " " +
                                                                                                     statusText
...
<script>
    function switchExampleResponse(
        endpointId,
        index,
    ) {
        document.querySelectorAll(`.example-response-${endpointId}`).forEach(el => el.style.display = "none")
        document.querySelectorAll(`.example-response-${endpointId}-${index}`)
    ...
    function toggleExpansionChevrons(evt) {
        ...
        const newState = chevronsArea.classList.contains("expanded")
                         ? "expand"
                         : "expanded"
        if (newState === "expanded") {
            const selector = solid
                             ? "#expanded-chevron-solid"
                             : "#expanded-chevron"
